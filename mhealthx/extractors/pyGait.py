import sys
sys.path.insert(1, '/Users/tanmay-s/Desktop/mhealthx/mhealthx')
import xio
import signals
"""
...
def heel_strikes(data, sample_rate, threshold=0.2, order=4, cutoff=5,
    ...
    filtered = signals.butter_lowpass_filter(data, sample_rate, cutoff, order)

    # Find transitional positions where AP accelerometer changes from
    # positive to negative.
    transitions = signals.crossings_nonzero_pos2neg(filtered)
    ...
    # Compute number of samples between peaks using the real part of the FFT:
    interpeak = signals.compute_interpeak(data, sample_rate)
...
def gait_regularity_symmetry(data, step_period, stride_period):
    ...
    coefficients, N = signals.autocorrelate(data, unbias=2, normalize=2,
                                    plot_test=False)

    step_regularity = coefficients[int(step_period)]
    stride_regularity = coefficients[int(stride_period)]
...
start = 0
device_motion = False
t, axyz, gxyz, uxyz, rxyz, sample_rate, duration = xio.read_accel_json(input_file, start, device_motion)
ax, ay, az = axyz
threshold = 0.2
order = 4
cutoff = 5
...
strikes, strike_indices = heel_strikes(data, sample_rate, threshold, order, cutoff, plot_test)
distance = 90
a = list(gait(strikes, data, duration, distance))
print(a)
